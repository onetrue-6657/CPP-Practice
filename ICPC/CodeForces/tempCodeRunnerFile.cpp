...
const int N = 200020;
int t, n, a[N];
int cnt0, cnt1, cnt2, res, len0, len1, len2;
...
void solve() {
    ...
    bool inc = true;
    cnt0 = 0, cnt1 = 0, cnt2 = 0, res = 0;
    for (int i = 1; i <= n; i++) {
        std::cin >> a[i];
        if (a[i] == 0) { cnt0++; pos0.push_back(i); }
        if (a[i] == 1) { cnt1++; pos1.push_back(i); }
        if (a[i] == 2) { cnt2++; pos2.push_back(i); }
    }   
